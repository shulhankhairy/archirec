	...
	class pemesanan3
	{
        ...
        function upload_image()
		{
			...
			if($denah != ""){
				...
				if(file_put_contents($path1, base64_decode($denah))){
			...
			if($foto_depan != ""){
				...
				if(file_put_contents($path2, base64_decode($foto_depan))){
					echo "uploaded_success";
					mysqli_close($this->connection);
				}else{
					echo "uploaded_failed";
			...
			if($foto_jalan != ""){ 
				...
				if(file_put_contents($path3, base64_decode($foto_jalan))){
					echo "uploaded_success";
					mysqli_close($this->connection);
				}else{
					echo "uploaded_failed";
			...
			if($foto_kanan != ""){
				...
				if(file_put_contents($path4, base64_decode($foto_kanan))){
					echo "uploaded_success";
					mysqli_close($this->connection);
				}else{
					echo "uploaded_failed";
			...
			if($foto_kiri != ""){
				...
				if(file_put_contents($path5, base64_decode($foto_kiri))){
					echo "uploaded_success";
				}else{
					echo "uploaded_failed";
				}
			...
			if($ruang != ""){
				...
				$update6 = mysqli_query($this->connection, "UPDATE PEMESANAN set PESAN_GBR_KEBUTUHAN_RUANG='$ruang' where PESAN_ID='$id'");
				if($update6){
				    echo "uploaded_success";
				}else{
				    echo "uploaded_failed";
				}
				/*if(file_put_contents($path6, base64_decode($ruang))){
					echo "uploaded_success";
				}else{
					echo "uploaded_failed";
				}*/
